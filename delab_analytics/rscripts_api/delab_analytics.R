...
function(texts = ""){
  ...
  if (any(hate_speech$hate_speech == 1)){
    print("Hate speech detected. Forwarding to moderation.")
  ...
  else
  {
    ...
    #response
    list(df = data.frame(out_llm), hate_speech_detected=FALSE, intervention_probability = prob_intervention, features = feats, feature_importance = out_importance)
  }
}